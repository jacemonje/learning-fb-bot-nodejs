...
bot.setGreetingText("Hello, I'm Jace Bot. I can help you manage your tasks. Be sure to setup your bucket by typing 'Setup'. ")

bot.setGetStartedButton((payload, chat) => {
  if(config.bucket === undefined){
    chat.say('Hello my name is Jace Bot and I serve as a note-taking app via Facebook Messenger')
...
bot.hear(['Get Started'], (payload, chat)=>{
  chat.getUserProfile().then((user) => {
    chat.say(`Hey ${user.first_name}! Welcome to Jace Bot`)
  })
})

bot.hear(['gago', 'pakyu', 'ulol'], (payload, chat)=>{
  chat.getUserProfile().then((user) => {
    chat.say(`Hoy ${user.last_name} wag ka magmura dito! `)
  })
})

bot.hear(['tangina', 'putangina', 'tangina mo', 'putangina mo'], (payload, chat)=>{
  chat.getUserProfile().then((user) => {
    chat.say(`tangina mo rin gago`)
  })
})

bot.hear(['taguro'], (payload, chat)=>{
  chat.getUserProfile().then((user) => {
    chat.say(`ANONG GENTLE GENTLE? Ilalabas ko na ang aking dragon! Dragon na maliit na may tatong sisiw`)
  })
})
...
bot.hear('create', (payload, chat) => {
  chat.conversation((convo) => {
    convo.ask("You have selected function-reminder! What would you like your reminder to be?", (payload, convo) => {
